...
predict_orbital_elements_t *predict_parse_omm(const char *omm_str)
{
    ...
    if (tokens)
    {
        ...
        if (ntokens >= 17)
        {
            ...
            if (sscanf(tokens[2], "%4d-%2d-%2dT%2d:%2d:%lf", &timeS.tm_year, &timeS.tm_mon, &timeS.tm_mday,
                       &timeS.tm_hour, &timeS.tm_min, &seconds) == 6)
            {
                timeS.tm_year -= 1900;
                timeS.tm_mon -= 1;
                double timestamp_full = timegm(&timeS) + seconds;

                struct tm timeS2;
                memset(&timeS2, 0, sizeof(struct tm));
                timeS2.tm_year = timeS.tm_year;
                double timestamp_year = timegm(&timeS2);

                m->epoch_year = timeS.tm_year - 100;
                m->epoch_day = ((timestamp_full - timestamp_year) / (3600. * 24.));
            }

            m->mean_motion = atof(tokens[3]);
            m->eccentricity = atof(tokens[4]);
            m->inclination = atof(tokens[5]);
            m->right_ascension = atof(tokens[6]);
            m->argument_of_perigee = atof(tokens[7]);
            m->mean_anomaly = atof(tokens[8]);

            m->satellite_number = atof(tokens[11]);
            m->element_number = atof(tokens[12]);
            m->revolutions_at_epoch = atof(tokens[13]);

            m->bstar_drag_term = atof(tokens[14]);
            m->derivative_mean_motion = atof(tokens[15]);
            m->second_derivative_mean_motion = atof(tokens[16]);

            memcpy(m->designator, "UNKNOW", 6);
        }
...
int predict_orbit(const predict_orbital_elements_t *orbital_elements, struct predict_position *m, double utc)
{
    ...
    m->revolutions =
        (long)floor((xno * MINUTES_PER_DAY / (M_PI * 2.0) + age * orbital_elements->bstar_drag_term) * age +
                    xmo / (2.0 * M_PI)) +
...
bool predict_decayed(const predict_orbital_elements_t *orbital_elements, predict_julian_date_t time)
{
    ...
    if (satepoch +
            ((16.666666 - orbital_elements->mean_motion) / (10.0 * fabs(orbital_elements->derivative_mean_motion))) <
        time)
    {
        has_decayed = true;
    }
...
double predict_squint_angle(const predict_observer_t *observer, const struct predict_position *orbit, double alon,
                            double alat)
{
